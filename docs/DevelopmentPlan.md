...
## 实施状态速览（2026-09-02 第一次开发收官标注）

**总体：Sprint 0–3 编码全部完成，Sprint 4 完成除月度报告外的全部编码；核心算法 21 个单测全绿，前端构建通过。端到端联调因数据库凭据未就绪而暂停。**

| 分类 | 内容 |
| :--- | :--- |
| ✅ 已完成 | 后端全部模块（脚手架/迁移/CRUD/行情适配/NAV 引擎/估值器/调度 Job/飞书+Telegram 推送/告警/指标/回补 CLI）；前端五页看板（总览/持仓/流水/复盘/设置）；`pytest` 21 个单测通过；`npm run build` 通过；备份脚本 `scripts/backup_db.sh`；`.gitignore` |
| ⬜ 未完成 | 任务 4.5（月度/年度报告 Job）；集成级端到端验证（建库 → 迁移 → uvicorn 冒烟 → 录数据 → 手动触发清算闭环）；备份恢复演练；真实 Webhook 推送验收 |
...
| 4 | 告警 + 复盘 + 收尾 | 第 7–8 周 | 告警规则触发可收到推送；月报含夏普/最大回撤；备份脚本可用 | 🚧 余 4.5 月度报告 Job 与端到端验收 |
...
| 3.2 | 总览页指标卡：最新 unit_nav、当日涨跌、累计收益、总资产 | 指标卡组件 | 与快照 API 数据一致；涨绿跌红 | ✅ |
...
| 4.5 | 月度/年度报告：胜率盈亏比、资产比例偏离预警（权益 >70% 警戒）、最佳/最差标的；每月最后交易日 20:00 推送 | 报告 Job | 生成一次真实月报卡片 | ⬜ 未开始 |
...
- **单测重点**（pytest）：NAV 平滑算法 4 场景（2.3）、四类估值器（2.2）、持仓重放（2.1）、固收按日计息边界。→ ✅ **已完成：`backend/tests/` 共 21 个用例全部通过**，另含「已知答案样例账本回放」回归基准（`test_replay.py`，5 日场景：初始建仓 / 内部出资抵消 / 纯波动 / 外部入金不扭曲 / 连续上涨）。
